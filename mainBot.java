    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event)
    {
        ...
        if(event.getMessage().getContentRaw().toLowerCase().startsWith(prefix+"admin"))
        {
            ...
            if(sender.getRoles().contains(roles.get(0)))
            {
                ...
                eb.addField("(prefix)createauc (name) {desc} {starting bid} (time in minutes)", " Creates auction with description", false);
                    ...
                    while(_m3>0) {
                        ...
                        TimerTask task = new TimerTask() {
                            ...
                            public void run() {
                                ...
                                if (commands.incrementTime(m1, final_m) == 0) {

                                    //Auction over
                                    EmbedBuilder eb = new EmbedBuilder();
                                    eb.setTitle("Auction Ended: ", null);
                                    eb.setColor(new Color(000000));
                                    eb.setDescription(m1);
        ...
        if(event.getMessage().getContentRaw().toLowerCase().startsWith(prefix+"bid"))
        {
            ...
            if(getArgs.length==3)
            {
                ...
                else
                {
                    ...
                    else if(Integer.parseInt(aucInfo[0])+1==Integer.parseInt(bid))
                    {
